...
public class SectionCrawler extends Thread {
	...
	/**
	 * Line to be parsed. Schools and majors of the line.
	 */
	...
	/**
	 * Add each section with {@code school} and {@code major} within {@code line}.
	 * 
	 * @param line
	 * @param school
	 * @param major
			...
			if (sectionID.length() > 0) {
				...
				if (times != null) {
					...
					if (days != null) {
						...
						int classCapacity = classCapacityString.contains(":") || classCapacityString.contains(" ") ? 0
								: Integer.parseInt(classCapacityString);
						if (classCapacity > 0) {
							String building_ID = tmp.contains("\"map\"")
									? getInnerHTMLByClassName(tmp, "map").substring(0, 3)
									: "TBA";

							Section section = new Section(sectionID, type, times[0], times[1], days, instructor,
									building_ID, classCapacity, courseID);
							// Add lecture id
	...
	private String[] processTime(String time) {
		...
		if (time.contains("TBA") || time.length() <= 0)
			return null;
	...
	private int addCourseToDB(String line, String school, String major) {
		...
		// Get number
		int startIndex = line.indexOf("<strong>" + major + " ") + (new String("<strong>" + major + " ")).length();
